...
and the length of stay (Table~\ref{tab:covariates}).
...
947 deaths have occurred and 328 patients have been censored (Table~\ref{tab:cohort}).
...
The overall Kaplan--Meier curve (Figure~\ref{fig:km-overall}) falls to about 0.53 at 36 months, most steeply
over the first few months; roughly three quarters of the cohort are alive at one year. Sex
and diabetes do not separate the cohort (Figure~\ref{fig:km-sex-dm}): one-year survival is 0.770 against
0.733 by sex and 0.740 against 0.764 by diabetes. Age and discharge BUN, split at their
medians, do (Figure~\ref{fig:km-age-bun}): 0.661 above age 76 against 0.844 below, and 0.676 above
...
potassium and \dbun{} (Table~\ref{tab:pa2cox}; in-sample concordance 0.694). Nine terms are significant:
...
predictions (Figure~\ref{fig:cox-aft}) are almost identical to the Cox model's for age, separate slightly
...
age with discharge BUN, the two strongest predictors in Table~\ref{tab:pa2cox}. Each is added as a single
...
stratified by the event indicator, so both sets carry the same event rate (Table~\ref{tab:split}): 3,168
...
Table~\ref{tab:hyper} gives the selections. Both networks took the smallest architecture on offer with the
...
Table~\ref{tab:coxtrain} refits the PA2 model on the training patients. The estimates are stable: no hazard
...
The Schoenfeld test gives a different answer on the training data (Table~\ref{tab:ph}). Discharge BUN
...
unremarkable among sixteen tests. The residual smoothers are flat throughout (Figure~\ref{fig:schoenfeld}).
...
Figure~\ref{fig:cox-oneyear} and Table~\ref{tab:coxpred} give predicted one-year survival against age, RDW and discharge BUN.
...
Figure~\ref{fig:model-curves} overlays all four models on the grids of Section~6.2 and Table~\ref{tab:modelcurves} quantifies them.
...
tails of Figure~\ref{fig:model-curves} from the other side: the forest cannot leave the
...
IBS sits slightly above the cross-validated values of Table~\ref{tab:hyper}, as expected on unseen
patients.

Calibration (Figure~\ref{fig:calibration}) plots each decile's mean prediction against its Kaplan--Meier
...
or accuracy by any margin this test set can detect. This follows from Figure~\ref{fig:model-curves} --- the models predict alike where
...
A shared C-index could mean agreement on the ranking or offsetting differences. Table~\ref{tab:risk}
...
71--78\%. Figure~\ref{fig:risk-tertiles} shows the cost. Cox risk tertiles give one-year survival of 0.892, 0.778
...
version of the discrimination gap in Table~\ref{tab:testperf}.

\subsection{Performance in clinically meaningful subgroups}

Table~\ref{tab:subgroup} scores the models inside the regions where Figure~\ref{fig:model-curves} showed most disagreement,
...
thinnest slice of events and is most likely noise. Discharge BUN separated the models most in Figure~\ref{fig:model-curves} yet produces no
...
$\times$ discharge BUN is not (0.29, $p = 0.59$; Table~\ref{tab:interactions}). The hazard ratio of 1.0001 is
...
on the other interacting covariate being zero, so that is not comparable with Table~\ref{tab:coxtrain}.
Neither interaction changes training concordance beyond the third decimal.

Figure~\ref{fig:interactions} shows why the two differ. The three curves in the left panel are not parallel: where
...
A significant term need not be a useful one. In Table~\ref{tab:interperf} the BUN interaction lowers test
...
Seven terms carry the model (Table~\ref{tab:coxtrain}): age, discharge BUN, RDW, BNP and length of stay raise
...
seed, and since the margins in Table~\ref{tab:testperf} are smaller than the test set can resolve, another
...
($n = 3{,}168$, 1,343 events), with the full-data estimates of Table~\ref{tab:pa2cox} for comparison. Rows
...
Table~\ref{tab:coxpred} exactly, confirming that all four are evaluated on the same grids and the same
