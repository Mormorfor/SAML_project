...
and the length of stay (Table~2).

The event is death from any cause and time is measured in months from discharge. Follow-up
closes administratively at 36 months, so patients still alive are right-censored there. Of
the 2,282 censored patients, 1,473 reach the cutoff and 809 are lost earlier, at a median
of 16.5 months. Censoring is assumed independent of survival time given the covariates.
Median follow-up is 25.6 months (IQR 8.8--36.0); by the one-year horizon used throughout,
947 deaths have occurred and 328 patients have been censored (Table~1).

Following PA1, missing continuous values were replaced by medians; no categorical covariate
had missing values. Following PA2, the nine comorbidity indicators were replaced by
comorbidity burden --- their count, grouped 0--1 (reference), 2--3 and 4+ --- and
\dbun{} $=$ BUN at discharge $-$ BUN on admission was constructed. The cohort is old and
comorbid: median age 76, 46.5\% women, hypertension in 86.6\% and ischaemic heart disease
in 62.7\%, with median BUN rising from 25\,mg/dL on admission to 28 at discharge.
...
The overall Kaplan--Meier curve (Figure~1) falls to about 0.53 at 36 months, most steeply
over the first few months; roughly three quarters of the cohort are alive at one year. Sex
and diabetes do not separate the cohort (Figure~A2): one-year survival is 0.770 against
0.733 by sex and 0.740 against 0.764 by diabetes. Age and discharge BUN, split at their
medians, do (Figure~2): 0.661 above age 76 against 0.844 below, and 0.676 above
28\,mg/dL against 0.810 below, with confidence bands clear of each other throughout.
...
potassium and \dbun{} (Table~3; in-sample concordance 0.694). Nine terms are significant:
age at 1.044 per year, discharge BUN at 1.021 per mg/dL, RDW at 1.120 per unit and BNP at
1.0002 per pg/mL, with sodium, potassium and \dbun{} below 1. Sex, comorbidity burden,
white blood cells, discharge creatinine, discharge GFR and platelets are not.

\dbun{} does add information beyond the discharge level: a likelihood-ratio test against
the model without it gave 27.63 on 1 degree of freedom, $p = 1.5 \times 10^{-7}$, so it is
retained throughout. A Schoenfeld residual test on a rank transform of time flagged two of
sixteen terms --- discharge BUN at 0.036 and length of stay at 0.048 --- both marginally,
so PA2 read the violations as mild.

A log-normal AFT model on the same covariates agreed on direction throughout. Its one-year
predictions (Figure~3) are almost identical to the Cox model's for age, separate slightly
for RDW, and differ most for discharge BUN, where the AFT curve falls faster in mid-range
and then crosses above in the tail. That figure sweeps BUN to its observed maximum of
236\,mg/dL while three quarters of patients sit below 39, which is why percentile grids are
used from Section~6.2 onwards.
...
age with discharge BUN, the two strongest predictors in Table~3. Each is added as a single
...
stratified by the event indicator, so both sets carry the same event rate (Table~4): 3,168
...
Table~5 gives the selections. Both networks took the smallest architecture on offer with the
...
test-set result. All four cross-validated scores lie within 0.0023 of one another, inside
...
Table~6 refits the PA2 model on the training patients. The estimates are stable: no hazard
...
The Schoenfeld test gives a different answer on the training data (Table~7). Discharge BUN
...
unremarkable among sixteen tests. The residual smoothers are flat throughout (Figure~A1).
...
Figure~4 and Table~8 give predicted one-year survival against age, RDW and discharge BUN.
...
Figure~5 overlays all four models on the grids of Section~6.2 and Table~9 quantifies them.
Over the interquartile range they agree: age moves predicted one-year survival by 0.09 to
0.12, discharge BUN by 0.03 to 0.07 and RDW by 0.03 to 0.06, and all four put age first.
...
0.462, DeepSurv to 0.512 and the forest to 0.719. Cox extrapolates its log-hazard slope
wherever the grid goes; the networks follow it to about 40\,mg/dL and then bend upwards; the
...
tails, and there all three are more conservative than the linear extrapolation.
...
nearly identical across models (0.748, 0.751, 0.745 and 0.745 for Cox, the forest, DeepSurv
...
199.7. These are the tails of Figure~5 from the other side: the forest cannot leave the
range its leaves cover, and Cox extrapolates without limit.

\begin{table}[H]
\centering
\caption*{\textbf{Table 10.} Test-set performance: Uno's C-index at $\tau = 12$ months, the
...
DeepSurv & 0.6904 & 0.1152 & 0.0337 \\
Cox-Time & 0.6909 & 0.1148 & 0.0198 \\
\bottomrule
\end{tabular}
\end{table}

Uno's C spans 0.015 (Cox 0.696, Cox-Time 0.691, DeepSurv 0.690, RSF 0.681) and the
integrated Brier score 0.0021 (Cox-Time 0.1148, Cox 0.1150, DeepSurv 0.1152, RSF 0.1169).
With 199 one-year deaths behind them, neither spread should be read as a ranking; what they
support is that no flexible model discriminates or predicts better than the Cox model. Test
IBS sits slightly above the cross-validated values of Table~5, as expected on unseen
patients.

Calibration (Figure~6) plots each decile's mean prediction against its Kaplan--Meier
survival. Deciles hold about 80 patients, so the intervals are roughly 0.2 wide. Cox-Time is
best calibrated, with a mean gap of 0.020, about half the others'. DeepSurv and Cox each
have one clear miss: DeepSurv predicts 0.658 for its third decile against 0.500 observed,
and Cox predicts 0.400 and 0.591 for its two lowest against 0.492 and 0.485. The forest
misses structurally instead --- its decile means span only 0.538 to 0.912 against 0.400 to
0.933 for Cox, with the middle deciles below the diagonal and the healthiest above it (0.912
against 0.950) --- so it is insufficiently confident about both good and bad prognoses.

No model wins on all three measures: Cox on the C-index, Cox-Time on IBS (by 0.0002) and
calibration, the forest last on all three. Every margin but the calibration one is inside
what this test set can resolve. For RQ1, then: one-year prediction from discharge
information sits at a C-index of about 0.69, and relaxing the Cox assumptions did not improve
discrimination or accuracy. This follows from Figure~5 --- the models predict alike where
...
A shared C-index could mean agreement on the ranking or offsetting differences. Table~11
shows agreement: Cox, DeepSurv and Cox-Time correlate at 0.97 or above and share 87--89\% of
their own top-risk quintile, while the forest correlates at 0.88--0.91 and shares only
71--78\%. Figure~7 shows the cost. Cox risk tertiles give one-year survival of 0.892, 0.778
...
separates them cleanly --- the patient-level version of the discrimination gap in Table~10.

\subsection{Performance in clinically meaningful subgroups}

Table~12 scores the models inside the regions where Figure~5 showed most disagreement,
splitting at the training medians so the subgroups do not depend on test data. Discharge BUN
does not overturn the ranking: Cox is best in both groups (0.714 and 0.636) and the forest
worst (0.695 and 0.617), though DeepSurv and Cox-Time edge ahead on IBS in the high-BUN
group (0.1437 against 0.1446), the only place a flexible model beats Cox on accuracy. Age
does overturn it: the forest discriminates best among patients 76 and younger (0.701 against
Cox's 0.666) and worst among the older (0.580 against 0.617). Only 62 of the 199 one-year
deaths fall in the younger group, so that reversal rests on the thinnest slice of events and
is most likely noise. Discharge BUN separated the models most in Figure~5 yet produces no
...
$\times$ discharge BUN is not (0.29, $p = 0.59$; Table~13). The hazard ratio of 1.0001 is
...
on the other interacting covariate being zero, so that is not comparable with Table~6.
Neither interaction changes training concordance beyond the third decimal.

Figure~8 shows why the two differ. The three curves in the left panel are not parallel: where
...
A significant term need not be a useful one. In Table~14 the BUN interaction lowers test
...
Seven terms carry the model (Table~6): age, discharge BUN, RDW, BNP and length of stay raise
...
Cox-Time rank individual test patients almost identically. Disagreement is confined to the
sparse tails and reflects models running out of data. The forest is the exception: it ranks
patients differently enough that the patients it calls mid risk and high risk have similar
outcomes.

No model is clearly best on the test set. Cox leads on Uno's C (0.696 against 0.691, 0.690
and 0.681), Cox-Time on the integrated Brier score (0.1148 against 0.1150, 0.1152 and
0.1169) and on calibration (0.020 against 0.034, 0.037 and 0.039). Every margin but the
calibration one is smaller than 199 one-year deaths can resolve. All four are adequately
calibrated at group level and the forest is consistently last, worst on discrimination in
three of the four subgroups.
...
proportional hazards buys no measurable discrimination or accuracy, with calibration the
only place flexibility shows. RQ3 is the one question where model choice matters, and there
...
seed, and since the margins in Table~10 are smaller than the test set can resolve, another
...
\caption*{\textbf{Figure 1.} Overall Kaplan--Meier survival curve after hospital discharge,
...
\caption*{\textbf{Figure 2.} Kaplan--Meier curves stratified at the median of age (left, 76
...
\caption*{\textbf{Figure 3.} Predicted one-year survival from the PA2 Cox model and the
...
\caption*{\textbf{Figure 4.} Predicted one-year survival from the Cox model fitted on the
...
\caption*{\textbf{Figure 5.} Predicted one-year survival as a function of age (top left),
...
\caption*{\textbf{Figure 6.} Calibration of the four models on the test set. Test patients
...
\caption*{\textbf{Figure 7.} Kaplan--Meier curves for the test set split into tertiles of
...
\caption*{\textbf{Figure 8.} Predicted one-year survival under each interaction model,
...
\caption*{\textbf{Figure A1.} Scaled Schoenfeld residuals against time since discharge for
...
\caption*{\textbf{Figure A2.} Kaplan--Meier curves stratified by sex (left) and by diabetes
...
\begin{table}[H]
\centering
\caption*{\textbf{Table 1.} Sample size, observed events and the composition of censoring.}
\vspace{0.3em}
\small
\begin{tabular}{lcc}
\toprule
...
\begin{table}[H]
\centering
\caption*{\textbf{Table 2.} Covariate summary for all 3,961 patients. Continuous variables
...
\begin{table}[H]
\centering
\caption*{\textbf{Table 3.} The Cox model selected in PA2, fitted to the full dataset
...
\begin{table}[H]
\centering
\caption*{\textbf{Table 4.} The 80/20 stratified train--test split at seed 42.}
\vspace{0.3em}
\small
\begin{tabular}{lcccc}
\toprule
...
\begin{table}[H]
\centering
\caption*{\textbf{Table 5.} Selected configuration for each model with its cross-validated
...
DeepSurv        & hidden [16], dropout 0.4, lr 0.001, 108 epochs   & 0.1093 & 0.0027 \\
Cox-Time        & hidden [16], dropout 0.4, lr 0.01, 24 epochs     & 0.1113 & 0.0033 \\
\bottomrule
\end{tabular}
\end{table}
...
\begin{table}[H]
\centering
\caption*{\textbf{Table 6.} The selected Cox model refitted on the training data
($n = 3{,}168$, 1,343 events), with the full-data estimates of Table~3 for comparison. Rows
...
\begin{table}[H]
\centering
\caption*{\textbf{Table 7.} Schoenfeld residual test for proportional hazards on a rank
...
\begin{table}[H]
\centering
\caption*{\textbf{Table 8.} Predicted one-year survival at the ends of each plotted grid and
...
\begin{table}[H]
\centering
\caption*{\textbf{Table 9.} Predicted one-year survival at the ends of each plotted grid and
across each covariate's interquartile range, for all four models. The Cox rows reproduce
Table~8 exactly, confirming that all four are evaluated on the same grids and the same
...
Age (years) & Cox      & 0.962 & 0.641 & 0.321 & 0.110 \\
            ...
            & DeepSurv & 0.944 & 0.594 & 0.350 & 0.108 \\
            & Cox-Time & 0.959 & 0.683 & 0.276 & 0.092 \\
\addlinespace
RDW (\%)    & Cox      & 0.864 & 0.659 & 0.205 & 0.034 \\
            & RSF      & 0.904 & 0.801 & 0.103 & 0.057 \\
            & DeepSurv & 0.887 & 0.719 & 0.168 & 0.045 \\
            & Cox-Time & 0.906 & 0.688 & 0.218 & 0.040 \\
\addlinespace
BUN, discharge & Cox      & 0.876 & 0.298 & 0.577 & 0.064 \\
               & RSF      & 0.899 & 0.719 & 0.180 & 0.032 \\
               & DeepSurv & 0.879 & 0.512 & 0.367 & 0.060 \\
               & Cox-Time & 0.912 & 0.462 & 0.450 & 0.068 \\
\bottomrule
\end{tabular}
\end{table}
...
\begin{table}[H]
\centering
\caption*{\textbf{Table 11a.} Spearman rank correlation between the four models' predicted
one-year risk on the test set.}
\vspace{0.3em}
\small
\begin{tabular}{lcccc}
\toprule
...
\midrule
Cox      & 1.000 & 0.884 & 0.974 & 0.971 \\
RSF      & 0.884 & 1.000 & 0.905 & 0.884 \\
DeepSurv & 0.974 & 0.905 & 1.000 & 0.971 \\
Cox-Time & 0.971 & 0.884 & 0.971 & 1.000 \\
\bottomrule
\end{tabular}
\end{table}
...
\begin{table}[H]
\centering
\caption*{\textbf{Table 11b.} Overlap between the top-risk quintiles of each pair of models:
159 test patients per model.}
\vspace{0.3em}
\small
\begin{tabular}{llccc}
\toprule
...
Cox      & DeepSurv & 140 & 88.1 & 0.787 \\
Cox      & Cox-Time & 142 & 89.3 & 0.807 \\
RSF      & DeepSurv & 124 & 78.0 & 0.639 \\
RSF      & Cox-Time & 114 & 71.7 & 0.559 \\
DeepSurv & Cox-Time & 139 & 87.4 & 0.777 \\
\bottomrule
\end{tabular}
\end{table}
...
\begin{table}[H]
\centering
\caption*{\textbf{Table 12.} Test-set discrimination and prediction error within subgroups
...
\midrule
BUN $\le$ 28 mg/dL & Uno's C & 430 (81) & 0.7140 & 0.6946 & 0.7026 & 0.7013 \\
                   & IBS     & 430 (81) & 0.0901 & 0.0933 & 0.0911 & 0.0904 \\
\addlinespace
BUN $>$ 28 mg/dL   & Uno's C & 363 (118) & 0.6361 & 0.6173 & 0.6339 & 0.6286 \\
                   & IBS     & 363 (118) & 0.1446 & 0.1449 & 0.1437 & 0.1437 \\
\addlinespace
Age $\le$ 76 years & Uno's C & 406 (62) & 0.6663 & 0.7010 & 0.6740 & 0.6735 \\
                   & IBS     & 406 (62) & 0.0787 & 0.0783 & 0.0787 & 0.0783 \\
\addlinespace
Age $>$ 76 years   & Uno's C & 387 (137) & 0.6172 & 0.5800 & 0.6075 & 0.6150 \\
                   & IBS     & 387 (137) & 0.1532 & 0.1574 & 0.1535 & 0.1531 \\
\bottomrule
\end{tabular}
\end{table}
...
\begin{table}[H]
\centering
\caption*{\textbf{Table 13.} Likelihood-ratio and Wald tests for the two pre-specified
...
\begin{table}[H]
\centering
\caption*{\textbf{Table 14.} Predictive performance of the additive Cox model and each
